...
void thongKeSiSoTheoBuoi(Vector<PhieuDiemDanh>& dsDD, const String& maLop) {
  ...
  for (int i = 0; i < (int)ngayDD.size(); ++i) {
    ...
    std::cout << "  " << std::left << std::setw(4)  << (i + 1)
              << std::setw(12) << ngayDD[i] << std::right
              << std::setw(9) << coMat
              << std::setw(11) << vangP
              << std::setw(12) << vangK
              << std::setw(10) << (vangP + vangK) << "\n";
  }
  printDynamicSeparator(58);
  if (ngayDD.size() == 0) {
...
void xuatBaoCaoFile(Vector<SinhVien>& dsSV, Vector<PhieuDiemDanh>& dsDD,
  ...
  String tenFile =
      String(exportDir) + String("baocao_") + maLop + String(".txt");
