...
// Tach chuoi theo ki tu phan cach
...
// Load du lieu lop hoc tu file txt
...
// Save / append du lieu xuong file txt
bool saveLopHoc(const char* path, Vector<LopHoc>& arr) {
  std::ofstream ofs(path);
  if (!ofs.is_open()) return false;
  for (int i = 0; i < (int)arr.size(); ++i) {
    LopHoc& lh = arr[i];
    ofs << lh.maLop           << '|' << lh.tenLop     << '|' 
        << lh.giangVien       << '|' << lh.tkb.thu    << '|' 
        << lh.tkb.tietBatDau  << '|' << lh.tkb.soTiet << '|'
