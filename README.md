...
| Module | File | Mô tả |
|--------|------|-------|
| **CustomString** | `CustomString.h/cpp` | Class `String` tự viết thay thế `std::string`. Hỗ trợ copy/move semantics, so sánh case-insensitive, nối chuỗi, `toInt()`, `substring()`, I/O stream. Sử dụng các hàm helper thay thế `<cstring>` (`my_strlen`, `my_memcpy`, `my_strcmp`). |
| **Vector** | `Vector.h` | Class template `Vector<T>` tự viết thay thế `std::vector`. Hỗ trợ `push_back` (copy & move), `remove`, `pop_back`, `clear`, copy/move assignment, range-based for loop (iterator). Chiến lược mở rộng: nhân đôi capacity. |
...
| **FileIO** | `FileIO.h/cpp` | Đọc/ghi dữ liệu từ file text dạng pipe-delimited (`|`). Hỗ trợ load/save cho 3 loại dữ liệu và append phiếu điểm danh. |
| **Search** | `Search.h/cpp` | Tìm kiếm tuyến tính: tìm lớp theo mã, tìm SV theo MSSV, lọc SV theo lớp, lọc điểm danh theo ngày/lớp/MSSV. Case-insensitive. |
| **Sort** | `Sort.h` | Thuật toán sắp xếp template: Hoare QuickSort + Median-of-Three pivot + Insertion Sort cho mảng nhỏ (cutoff = 16). Hỗ trợ custom comparator (lambda/functor). |
| **Validation** | `Validation.h/cpp` | Kiểm tra đầu vào: ngày tháng (DD/MM/YYYY có kiểm tra năm nhuận), MSSV (chữ+số, ≤ 20 ký tự), mã lớp, trạng thái điểm danh. Các hàm nhập liệu an toàn với vòng lặp retry. |
